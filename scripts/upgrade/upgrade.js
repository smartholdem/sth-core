...
const main = async () => {
    let {
        ...
        type: 'text',
        name: 'corePath',
        initial: expandHomeDir('~/sth-core'),
    ...
    fs.writeFileSync(`${paths.config.new}/.env`, envCurrent.replace(new RegExp('STH_', 'g'), 'CORE_'));

    // Update plugins file
    console.log('Update plugins configuration');
    let pluginContents = fs.readFileSync(`${paths.config.new}/plugins.js`).toString();
    pluginContents = pluginContents.replace('@smartholdem/core-transaction-pool-mem', '@smartholdem/core-transaction-pool');
    pluginContents = pluginContents.replace('"@smartholdem/core-config": {},', '');
    pluginContents = pluginContents.replace("'@smartholdem/core-config': {},", '');
    pluginContents = pluginContents.replace(new RegExp('STH_', 'g'), 'CORE_');
