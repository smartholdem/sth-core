import { DatabaseService, Repositories } from "@smartholdem/core-database";
import { Container, Contracts, Enums, Providers, Types, Utils } from "@smartholdem/core-kernel";
import { DatabaseInteraction } from "@smartholdem/core-state";
import { Blocks, Crypto, Interfaces, Managers } from "@smartholdem/crypto";
...
export class Blockchain implements Contracts.Blockchain.Blockchain {
    ...
    @Container.tagged("plugin", "@smartholdem/core-blockchain")
    ...
    public async initialize(): Promise<void> {
        ...
        if (this.stateStore.getNetworkStart()) {
            this.logger.warning(
                "SmartHoldem Core is launched in Genesis Start mode. This is usually for starting the first node on the blockchain. Unless you know what you are doing, this is likely wrong.",
            );
        }
