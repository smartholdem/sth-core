import { Commands, Container, Contracts, Services } from "@smartholdem/core-cli";
import { Blocks, Crypto, Identities, Interfaces, Managers, Transactions, Utils } from "@smartholdem/crypto";
...
export class Command extends Commands.Command {
    ...
    private generateCryptoMilestones(options: Options) {
        ...
        return [
            ...
            },
            {
                ...
                fees: {
                    staticFees: {
                        delegateRegistration: options.feeStaticDelegateRegistration * 10
                    }
                }
            }
        ];
    ...
    private generateCryptoGenesisBlock(genesisWallet, delegates, options: Options) {
        ...
        if (options.distribute) {
            ...
            console.log("premineWallet", premineWallet);
            console.log("genesisWallet", genesisWallet);
            console.log("premine", options.premine);
            console.log("pubKeyHash", options.pubKeyHash);
        }
    ...
    private generateApp(options: Options): any {
        ...
        if (includeDynamicFees) {
            app.core.plugins.find((plugin) => plugin.package === "@smartholdem/core-transaction-pool").options = {
                dynamicFees
            };

            app.relay.plugins.find((plugin) => plugin.package === "@smartholdem/core-transaction-pool").options = {
                dynamicFees
            };
        }
    ...
    private generateCoreDelegates(activeDelegates: number, pubKeyHash: number): Wallet[] {
        ...
        for (let i = 0; i < activeDelegates; i++) {
            ...
            delegateWallet.username = users[i];
